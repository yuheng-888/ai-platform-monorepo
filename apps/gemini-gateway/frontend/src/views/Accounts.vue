      ...
      <div class="ui-surface flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden">
        ...
        <div class="scrollbar-slim flex-1 overflow-y-auto px-6 py-4">
          ...
          <div v-if="addMode === 'register'" class="space-y-4">
            ...
            />
            <label class="block text-xs text-muted-foreground">注册并发</label>
            <input
          ...
          <div v-if="automationError || registerTask || loginTask" class="px-6 py-4">
            ...
            <div v-if="registerTask" class="space-y-1">
              ...
              <div class="flex flex-wrap gap-x-4 gap-y-1">
                ...
                <span>并发：{{ registerTask.concurrency || 1 }}</span>
...
const getDefaultRegisterCount = () => {
  const value = Number(settings.value?.basic?.register_default_count)
  return Number.isFinite(value) && value > 0 ? value : 1
}

const getDefaultRegisterConcurrency = () => {
  const requested = Number(settings.value?.basic?.register_default_concurrency)
  const maxValue = Number(settings.value?.basic?.register_max_concurrency)
  const normalizedMax = Number.isFinite(maxValue) && maxValue > 0 ? maxValue : 10
...
const registerCount = ref(getDefaultRegisterCount())
const registerConcurrency = ref(getDefaultRegisterConcurrency())
...
const openRegisterModal = () => {
  ...
  registerCount.value = getDefaultRegisterCount()
  registerConcurrency.value = getDefaultRegisterConcurrency()
  // 重置为设置中的邮箱服务提供商
...
const fetchTaskHistory = async () => {
  ...
  try {
    const response = await fetch(`${geminiApiBase}/admin/task-history`, {
...
const clearTaskHistory = async () => {
  ...
  if (!confirmed) return
  try {
    const response = await fetch(`${geminiApiBase}/admin/task-history?confirm=yes`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' }
    })
