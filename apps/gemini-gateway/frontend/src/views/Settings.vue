            ...
            <div class="ui-card">
              ...
              <div class="mt-4 space-y-3">
                ...
                <input
                  ...
                  class="ui-input-sm w-full"
                />
                <label class="block text-xs text-muted-foreground">默认注册并发</label>
                <input
                  v-model.number="localSettings.basic.register_default_concurrency"
                  type="number"
                  min="1"
                  class="ui-input-sm w-full"
                />
                <label class="block text-xs text-muted-foreground">注册最大并发</label>
                <input
                  v-model.number="localSettings.basic.register_max_concurrency"
                  type="number"
                  min="1"
                  class="ui-input-sm w-full"
                />
...
watch(settings, (value) => {
  ...
  next.basic.register_default_count = Number.isFinite(next.basic.register_default_count)
    ...
    : 1
  next.basic.register_default_concurrency = Number.isFinite(next.basic.register_default_concurrency)
    ? next.basic.register_default_concurrency
    : 1
  next.basic.register_max_concurrency = Number.isFinite(next.basic.register_max_concurrency)
    ? next.basic.register_max_concurrency
    : 10
  if (next.basic.register_max_concurrency < 1) {
    next.basic.register_max_concurrency = 10
  }
  if (next.basic.register_default_concurrency < 1) {
    next.basic.register_default_concurrency = 1
  }
  next.basic.register_default_concurrency = Math.min(
    next.basic.register_default_concurrency,
    next.basic.register_max_concurrency,
  )
...
const handleSave = async () => {
  ...
  try {
    ...
    localSettings.value.basic.register_default_count = Number.isFinite(localSettings.value.basic.register_default_count)
      ? Math.max(1, Number(localSettings.value.basic.register_default_count))
      : 1
    localSettings.value.basic.register_max_concurrency = Number.isFinite(localSettings.value.basic.register_max_concurrency)
      ? Math.max(1, Number(localSettings.value.basic.register_max_concurrency))
      : 10
    localSettings.value.basic.register_default_concurrency = Number.isFinite(localSettings.value.basic.register_default_concurrency)
      ? Math.max(1, Number(localSettings.value.basic.register_default_concurrency))
      : 1
    localSettings.value.basic.register_default_concurrency = Math.min(
      localSettings.value.basic.register_default_concurrency,
      localSettings.value.basic.register_max_concurrency,
    )
