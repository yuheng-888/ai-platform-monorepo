...
from embedded.gemini_business2api.util.streaming_parser import parse_json_array_stream_async
...
# 导入认证模块
from embedded.gemini_business2api.core.auth import verify_api_key
from embedded.gemini_business2api.core.session_auth import is_logged_in, login_user, logout_user, require_login, generate_session_secret

# 导入核心模块
from embedded.gemini_business2api.core.message import (
...
from embedded.gemini_business2api.core.account import (
    AccountManager,
    MultiAccountManager,
    RetryPolicy,
    CooldownConfig,
...
from embedded.gemini_business2api.core.version import get_update_status, get_version_info

# 导入 Uptime 追踪器
from embedded.gemini_business2api.core import uptime as uptime_tracker

# 导入配置管理和模板系统
from embedded.gemini_business2api.core.config import config_manager, config

# 数据库存储支持
from embedded.gemini_business2api.core import storage, account
...
try:
    from embedded.gemini_business2api.core.child_reaper import install_child_reaper
...
try:
    from embedded.gemini_business2api.core.register_service import RegisterService
    from embedded.gemini_business2api.core.login_service import LoginService
...
async def admin_start_register(
    ...
    concurrency: Optional[int] = Body(default=None),
):
...
async def admin_get_settings(request: Request):
    ...
    return {
        "basic": {
            ...
            "register_default_concurrency": config.basic.register_default_concurrency,
            "register_max_concurrency": config.basic.register_max_concurrency,
...
async def admin_update_settings(request: Request, new_settings: dict = Body(...)):
    ...
    try:
        ...
        basic.setdefault("register_default_concurrency", config.basic.register_default_concurrency)
        basic.setdefault("register_max_concurrency", config.basic.register_max_concurrency)
