...
function resolveDefaultConcurrency(cfg: Record<string, any>) {
  const raw = Number.parseInt(String(cfg.register_max_concurrency ?? ''), 10)
  return Number.isFinite(raw) && raw > 0 ? raw : FALLBACK_CONCURRENCY
}
...
export default function Accounts() {
  ...
  const [defaultConcurrency, setDefaultConcurrency] = useState(FALLBACK_CONCURRENCY)
      ...
      const res = await apiFetch('/tasks/register', {
        ...
        body: JSON.stringify({
          ...
          extra: {
            ...
            duckmail_verify_ssl: cfg.duckmail_verify_ssl,
  ...
  const openRegisterModal = async () => {
    ...
    try {
      const cfg = await apiFetch('/config')
      const nextConcurrency = resolveDefaultConcurrency(cfg)
      setDefaultConcurrency(nextConcurrency)
      ...
      setDefaultConcurrency(FALLBACK_CONCURRENCY)
      registerForm.setFieldsValue({
        count: 1,
        ...
        {!taskId ? (
          <Form form={registerForm} layout="vertical" onFinish={handleRegister}>
            ...
            <Form.Item
              name="concurrency"
              label="并发数"
              initialValue={defaultConcurrency}
              rules={[{ required: true }]}
