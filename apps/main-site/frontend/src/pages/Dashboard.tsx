...
export default function Dashboard() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [statsData, restockData, poolData] = await Promise.all([
        apiFetch('/accounts/stats'),
        apiFetch('/accounts/chatgpt-restock-summary'),
        apiFetch('/accounts/cliproxyapi-pool-summary'),
      ])
      setStats(statsData)
        ...
        <Col xs={24}>
          <Card title="CLIProxyAPI 号池">
            {loading ? (
              ...
              <Space size={24} wrap>
                <Statistic title="总数" value={poolSummary?.total ?? 0} />
                <Statistic title="启用数量" value={poolSummary?.enabled ?? 0} />
                <Statistic title="待删除封禁账号" value={poolSummary?.pending_delete_banned ?? 0} />
                <Statistic title="历史总封禁" value={poolSummary?.historical_total_banned ?? 0} />
                <div>
                  <Typography.Text type="secondary">同步状态</Typography.Text>
                  <div style={{ marginTop: 8 }}>
                    <Tag color={poolSummary?.last_error ? 'error' : 'success'}>
                      {poolSummary?.last_error ? '统计异常' : '统计正常'}
                    </Tag>
                    {poolSummary?.last_error ? (
        ...
        <Col xs={24}>
          <Card title="ChatGPT 自动补货">
            {loading ? (
              ...
              <Space size={24} wrap>
                <Statistic title="可用库存" value={restock?.available ?? 0} />
                <Statistic title="触发阈值" value={restock?.threshold ?? 0} />
                <Statistic title="目标库存" value={restock?.target ?? 0} />
                <Statistic title="每轮补货数" value={restock?.batch_size ?? 0} />
                <div>
                  <Typography.Text type="secondary">自动补货状态</Typography.Text>
                  <div style={{ marginTop: 8 }}>
                    <Tag color={restock?.enabled ? 'green' : 'default'}>
                      {restock?.enabled ? '已开启' : '未开启'}
                    </Tag>
