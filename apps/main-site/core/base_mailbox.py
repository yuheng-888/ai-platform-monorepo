...
def create_mailbox(provider: str, extra: dict = None, proxy: str = None) -> 'BaseMailbox':
    ...
    if provider == "tempmail_lol":
        ...
        return MoeMailMailbox(
            api_url=extra.get("moemail_api_url", "https://moemail.app"),
...
class MoeMailMailbox(BaseMailbox):
    """MoeMail 邮箱服务 - 使用 X-API-Key 调用官方 API"""

    def __init__(
        self,
        api_url: str = "https://moemail.app",
        api_key: str = "",
        domain: str = "",
        proxy: str = None,
    ):
        self.api = (api_url or "https://moemail.app").rstrip("/")
    ...
    def _parse_domains(self, data: dict) -> list:
        raw_domains = data.get("emailDomains") or data.get("domains") or []
        if isinstance(raw_domains, str):
            return [item.strip() for item in raw_domains.split(",") if item.strip()]
    ...
    def _get_available_domains(self) -> list:
        ...
        try:
            response = session.get(f"{self.api}/api/config", timeout=10)
    ...
    def _pick_domain(self) -> str:
        ...
        return random.choice(domains) if domains else "moemail.app"
    ...
    def _extract_messages(self, data: Any) -> list:
    ...
    def _parse_message_time(self, msg: dict) -> Optional[float]:
    ...
    def _fetch_message_detail(self, email_id: str, message_id: str) -> dict:
        session = self._ensure_session()
        try:
            response = session.get(f"{self.api}/api/emails/{email_id}/{message_id}", timeout=10)
            data = response.json() if hasattr(response, "json") else {}
    ...
    def _collect_message_text(self, email_id: str, msg: dict) -> str:
        import re
        ...
        body_parts = [
            ...
            str(msg.get("body") or ""),
            str(msg.get("html") or ""),
            str(msg.get("textContent") or ""),
            str(msg.get("htmlContent") or ""),
        ]
        text = " ".join(part for part in ([subject] + body_parts) if part).strip()
        if not any(part for part in body_parts) and msg.get("id"):
            detail = self._fetch_message_detail(email_id, str(msg.get("id")))
            if detail:
                detail_parts = [
                    ...
                    str(detail.get("textContent") or ""),
                    str(detail.get("htmlContent") or ""),
                ]
                text = " ".join(part for part in detail_parts if part).strip()
    ...
    def get_email(self) -> MailboxAccount:
        ...
        response = session.post(f"{self.api}/api/emails/generate", json=payload, timeout=15)
        data = response.json() if hasattr(response, "json") else {}
        self._email = data.get("email") or data.get("address") or ""
        email_id = str(data.get("id") or data.get("emailId") or "")
        print(f"[MoeMail] 生成邮箱: {self._email} id={email_id} domain={domain} status={response.status_code}")
        if not self._email or not email_id:
            raise RuntimeError(f"MoeMail 生成邮箱失败: {data}")
        return MailboxAccount(
            email=self._email,
            account_id=email_id,
            extra={"email_id": email_id, "domain": domain},
        )

    def get_current_ids(self, account: MailboxAccount) -> set:
        session = self._ensure_session()
        try:
            response = session.get(f"{self.api}/api/emails/{account.account_id}", timeout=10)
            data = response.json() if hasattr(response, "json") else {}
            return {
                str(message.get("id", ""))
                for message in self._extract_messages(data)
                if message.get("id")
            }
        except Exception:
            return set()
        ...
        **kwargs,
        ...
        while time.time() - start < timeout:
            try:
                ...
                messages = sorted(
                    messages,
                    key=lambda item: self._parse_message_time(item) or 0,
                    reverse=True,
                )
                for msg in messages:
                    ...
                    if otp_cutoff and message_time and message_time < otp_cutoff:
                        continue

                    body = self._collect_message_text(account.account_id, msg)
